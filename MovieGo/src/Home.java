...
public class Home extends javax.swing.JFrame {
    ...
    if (selectedRow != -1) {
        String filmID = tableMovie.getValueAt(selectedRow, 0).toString(); // Film Adı

        int choice = JOptionPane.showConfirmDialog(this,"Bu filme bilet almak istiyor musunuz?","Bilet Al",JOptionPane.YES_NO_OPTION);

        if (choice == JOptionPane.YES_OPTION) {
            // Kullanıcı adı sistemden alınmalı, örneğin giriş ekranından
            String kullaniciAdi = Login.kullanici_adi; // Örnektir, senin sistemine göre değişebilir

            // Bilet alma sorgusu
            String query = "INSERT INTO biletler (kullanici_adi, film_adi) " +
                           "VALUES ('" + kullaniciAdi + "','" + filmID + "')";
            InsertUpdateDelete.setData(query, "Bilet başarıyla alındı!");
        }
    }
